...
for _ in range(6):
    ...
    if got: break
print("hello ->","ACK" if got else "SIN ack - revisa WiFi/DJI Fly")
if not got: sys.exit()
for h in INIT:
...
def land():
    print(">>> ATERRIZANDO: throttle ABAJO. Mira el dron descender...",flush=True)
    run(12.0,3,1024,1024,364,1024,label="")
    print(">>> Fin del aterrizaje. Si sigue en el aire: MATA el script (Ctrl+C) -> failsafe Aterrizar, o usa el boton del dron.",flush=True)
if not FLY:
    run(5.0,3,1024,1024,1024,1024,label="DRY RUN (sin 'fly'): sesion+init+autoridad+neutro. NO despega.")
    print("DRY RUN ok, sesion viva. Para volar de verdad: python flight1.py fly")
    s.close(); sys.exit()
print("="*56)
print(" VUELO REAL. Requisitos: PADRE presente, EXTERIOR abierto,")
print(" failsafe=Aterrizar, altura max baja. Ctrl+C = ATERRIZAR.")
print("="*56,flush=True)
try:
    run(3.0,2,1024,1024,1024,1024,label="1) settle autoridad-02 + neutro")
    run(2.0,3,1024,1024,1024,1024,label="2) settle autoridad-03 + neutro")
    print(">>> DESPEGUE en 5 s (Ctrl+C para abortar)...",flush=True)
    for i in range(5,0,-1): print("  ",i,flush=True); time.sleep(1.0)
    run(5.5,3,1024,1024,1024,1024,fire=3,label="3) DESPEGUE + HOVER ~5s (neutro, deberia subir y flotar)")
    land()
except KeyboardInterrupt:
    print("\n!! ABORTO manual -> aterrizando",flush=True)
    try: land()
    except KeyboardInterrupt: print("Saliendo. Failsafe=Aterrizar deberia bajarlo; si no, boton del dron.")
finally:
    s.close()
print("FIN del vuelo.")
