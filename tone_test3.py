...
CV51=bytes.fromhex("5567048e3be901000051015544041aeee973000051130504020032303230656534642d366163612d343636662d00010101000000000000010000010101020000004afe320000009041540000005dd60099d4ac0201000000ffffffff018200000000000000cce1")
...
def f20(seq,st,ctr):
    ...
    return b+crc16(b).to_bytes(2,'little')
def modecmd(seq,mode=0x09):   # 0x03/0xf9 set-modo; 09 = Control manual
    b=bytes([0x55,0x15,0x04,0xa9,0x02,0x17])+(seq&0xffff).to_bytes(2,'little')+bytes([0x40,0x03,0xf9,0x87,0x88,0x67,0xa3,mode&0xff,0,0,0])
    return b+crc16(b).to_bytes(2,'little')
def tf(seq):
    b=bytes([0x55,0x12,0x04,0xc7,0x02,0x03])+(seq&0xffff).to_bytes(2,'little')+bytes([0x40,0x03,0xda,0x05,0xff,0xff,0xff,0xff])
    return b+crc16(b).to_bytes(2,'little')
s=socket.socket(socket.AF_INET,socket.SOCK_DGRAM); s.bind(("0.0.0.0",0)); s.settimeout(0.004)
got=False
for _ in range(6):
...
n=1; ctr=0x6a545000; ii=0
def run(dur,st,r,p,th,y,fire_hz=0,setmode=False,label=""):
    global n,ctr,ii
    if label: print(label,flush=True)
    end=time.time()+dur; nc=0.0; nf=0.0; nh=0.0; nfi=0.0; ni=0.0; n51=0.0; nm=0.0
    while time.time()<end:
        now=time.time()
        if now>=nc: s.sendto(wf(n,sf(n,r,p,th,y)),D); n+=1; nc=now+0.05
        if now>=n51: s.sendto(wf(n,CV51),D); n+=1; n51=now+0.03
        if now>=ni: s.sendto(wf(n,bytes.fromhex(INIT[ii%len(INIT)])),D); n+=1; ii+=1; ni=now+0.12
        if now>=nf: s.sendto(wf(n,f20(n,st,ctr)),D); n+=1; ctr+=1; nf=now+1.0
        if setmode and now>=nm: s.sendto(wf(n,modecmd(n,0x09)),D); n+=1; nm=now+0.5   # set manual repetido
        if fire_hz and now>=nfi: s.sendto(wf(n,tf(n)),D); n+=1; nfi=now+1.0/fire_hz
        if now>=nh: s.sendto(HELLO,D); nh=now+0.5
        try: s.recvfrom(2048)
        except socket.timeout: pass
        except BlockingIOError: pass
print("== TONE TEST v3 (+ modo MANUAL 0x09): dron ASEGURADO, escucha el tono ==",flush=True)
# mandar el set-modo manual varias veces primero
for k in range(4):
    s.sendto(wf(n,modecmd(n,0x09)),D); n+=1; time.sleep(0.05)
print(">>> set MODO MANUAL enviado",flush=True)
run(4.0,2,1024,1024,1024,1024,setmode=True,label="1) settle-02 + streams + set-manual")
run(4.0,3,1024,1024,1024,1024,setmode=True,label="2) settle-03 + streams + set-manual")
run(2.5,3,1024,1024,1024,1024,fire_hz=6,label="3) >>> DESPEGUE SOSTENIDO - ESCUCHA EL TONO <<<")
run(3.0,3,1024,1024,364,1024,label="4) throttle ABAJO (por si giro)")
s.close()
print("FIN tone test v3.",flush=True)
