...
### EXP-026 — El stick estaba TRUNCADO: faltaba el contador del stream de control (2026-07-14)
- Estado: [OBSERVED] — corrige el builder del stick; candidato fuerte al ultimo bloqueo del armado.
- Contexto: con el enganche 0x51 (EXP-025), `flight.py` lee ahora el OSD del FC en su propia sesion (`read_osd`/`report_fc`). El DRY reporta: **`estado=GPS_Atti, motores=False, MOTIVO NO-ARRANQUE=None/Allow start`** — el FC esta enganchado, nos procesa y **permite arrancar motores**. Y decodificando el OSD de la app (Octava) alrededor de su AUTO_FLY (t+19.2): el estado es **identico** (GPS_Atti, allow start) y el bit `motor_on` pasa a True exactamente en t+19.32 (valida la decodificacion). => el bloqueo NO es estado ni precondicion; es especifico de AUTO_FLY.
- **Hallazgo (comparando el stick 0x01/0x0a de la app vs el nuestro):** el nuestro estaba **TRUNCADO**. La app manda 28 bytes; nosotros 18. Faltaban 10 bytes al final: `56 08` + **contador u16 LE que INCREMENTA** (timestamp ms: a6e7->d9e7->0ae8...) + `00*6`. Los primeros 18 bytes coinciden.
- [INFERRED] El FC probablemente usa ese contador para validar que el **stream de control esta vivo y es monotono** (= controlador activo). Sin el, nuestro stick es descartado -> no somos 'controlador activo' -> el FC no autoriza AUTO_FLY aunque los motores esten permitidos. Explica ademas por que los sticks NUNCA respondieron (EXP-017).
- Cambio: `flight.py` `stick()` ahora emite la cola completa `...55 01 04 56 08 <ctr u16 LE ms> 00*6` (28 bytes, = la app salvo el valor del contador). `read_osd()` ademas mantiene sticks NEUTRO vivos (~20Hz) para no cortar el control antes de AUTO_FLY. Nuevos decodificadores OSD en `neo_udp.py` (`decode_osd_general`, `find_osd_general`, enums FLYC_STATE/START_FAIL).
- **Siguiente (gated, lo teclea el usuario):** `--fly` con el stick corregido. Si el FC ahora nos ve como controlador activo, AUTO_FLY deberia armar (motores permitidos + estado correcto + control vivo). Si no, el siguiente candidato es el heartbeat `0x03/0xd7` continuo (la app lo streamea; nosotros no por defecto).

- **RESULTADO EN HARDWARE (2026-07-14): ✅✅✅ EL NEO DESPEGO CON NUESTRO SOFTWARE.** `flight.py --fly` (via neo.ps1): ENGANCHE 0x51 OK, OSD `GPS_Atti / None/Allow start`, y tras AUTO_FLY el dron **arranco motores y despego a su altura normal de auto-despegue**, floto y respondio. El contador del stick (EXP-026) era la pieza que faltaba. **HITO CENTRAL DEL PROYECTO LOGRADO** (prioridad absoluta 2026-07-10): control programatico reproducible del DJI Neo original desde software propio -> [CONFIRMED PATH]. Cadena completa: WiFi (neo.ps1) -> transporte UDP fiable -> envoltorio 0x51/01 -> suscripcion 0x51 (engancha FC) -> stream de control vivo (stick con contador) -> FunctionControl AUTO_FLY.
- **PENDIENTE — aterrizaje:** el `AUTO_LANDING` (0x03/0x2a:02) NO hizo descender al dron; siguio flotando. El usuario aterrizo con la mano enguantada bajo el dron (palm-land por sensor inferior). Candidatos: (a) latch `function_command_state` no limpio tras AUTO_FLY -> AUTO_LANDING queda no-op (el dissector: "does nothing if function_command_state was already non-zero"); (b) AUTO_LANDING necesita repetirse/mantenerse o los sticks a throttle-min; (c) timing (mandamos land ~4s tras despegar). Siguiente: comparar la fase de aterrizaje de la app (Octava t+31.3) y/o usar descenso por throttle-min como fallback fiable. Failsafe/boton/palm-land funcionan como respaldo.
