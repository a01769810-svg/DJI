...
12. **`[OBSERVED, DECISIVO 2026-07-13]` El transporte YA no es el problema; el candado del ARMADO es un canal de control de vuelo firmado/cifrado por sesión.** Con el fix de flow-control de EXP-018, el cambio de **modo** (0x03/0xf9) sí ejecuta en hardware — pero el **despegue no**. La captura de un despegue real en Manual (EXP-019, "Octava prueba", + comparación con "Quinta") resolvió por qué:
    ...
    - **Feasibilidad (abierta):** el token `f2458601ac9a06fa` aparece **solo en el uplink, nunca en el downlink del dron** → la app lo genera/deriva, no nos lo entrega en claro. Si el dron lo acepta como nonce libre → reproducible; si lo valida contra una firma con secreto de dispositivo → **bloqueo duro** (consistente con el canal DUML firmado de la generación 2024). **NO se puede decidir con replay estático:** los payloads por-sesión lo invalidan. Herramientas: `tools/neo_control/analysis/unwrap.py` (des-envuelve 0x51/0x01 y saca el censo/timeline real de comandos internos). Detalle en [`EXPERIMENT_LOG.md`](EXPERIMENT_LOG.md) EXP-019.
