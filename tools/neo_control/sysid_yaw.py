...
def export_xlsx(csv_path, xlsx_path, ts=0.1):
    ...
    info = [
        ...
        ("", ""),
        ("MODELO SUGERIDO", "stick --[zona muerta]--> yaw_rate --[1/s]--> yaw_angle"),
        ("", "=> G(s) ~ K/(s(tau*s+1)): la planta lleva un INTEGRADOR. El FC trata el stick"),
        ("", "   como comando de VELOCIDAD (stick constante => giro a velocidad constante)."),
        ("", "Al ser integrador, un P/PD ya da error nulo a escalon: la I no suele hacer falta."),
        ("", ""),
        ...
        ("", ""),
        ("FASE A", "escalera de escalones -> curva ESTATICA (zona muerta + ganancia)"),
        ("FASE B", "PRBS bang-bang a saturacion -> DINAMICA (tau)"),
        ("ZONA MUERTA", "estimada a priori d~75, k~0.034 deg/s por unidad (de solo 2 puntos)."),
        ("", "La FASE A es la que la mide de verdad. NO afecta al PRBS: al ser bang-bang,"),
        ("", "|u| = %d siempre y la atraviesa sin reposar en ella." % MAX_DEFL),
        ("", ""),
    ]
    for k, v in meta.items():
