...
# frame 0x51/0x01 (103B) capturado del vuelo real; se streamea como hacia la app (~43Hz)
...
def run(dur,st,r,p,th,y,fire_hz=0,label=""):
    global n,ctr,ii
    if label: print(label,flush=True)
    end=time.time()+dur; nc=0.0; nf=0.0; nh=0.0; nfi=0.0; ni=0.0; n51=0.0
    while time.time()<end:
        now=time.time()
        if now>=nc: s.sendto(wf(n,sf(n,r,p,th,y)),D); n+=1; nc=now+0.05           # sticks 20Hz
        if now>=n51: s.sendto(wf(n,CV51),D); n+=1; n51=now+0.03                    # 0x51/0x01 ~33Hz
        if now>=ni: s.sendto(wf(n,bytes.fromhex(INIT[ii%len(INIT)])),D); n+=1; ii+=1; ni=now+0.12  # init rotando
        if now>=nf: s.sendto(wf(n,f20(n,st,ctr)),D); n+=1; ctr+=1; nf=now+1.0      # autoridad 1Hz
        if fire_hz and now>=nfi: s.sendto(wf(n,tf(n)),D); n+=1; nfi=now+1.0/fire_hz # despegue sostenido
        if now>=nh: s.sendto(HELLO,D); nh=now+0.5
        try: s.recvfrom(2048)
        except socket.timeout: pass
        except BlockingIOError: pass
print("== TONE TEST v2 (streaming completo): dron ASEGURADO, escucha el tono ==",flush=True)
run(4.0,2,1024,1024,1024,1024,label="1) settle autoridad-02 + streams (4s)")
run(4.0,3,1024,1024,1024,1024,label="2) settle autoridad-03 + streams (4s)")
...
s.close()
print("FIN tone test v2.",flush=True)
